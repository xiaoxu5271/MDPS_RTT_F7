...
#ifdef PA_LNA_MODE
    // Set up GPIO PA LNA gpio ctrl
    uint32 gpio_reg;
    gpio_reg = dwt_read32bitreg(GPIO_CTRL_ID); // read the current GPIO_CTRL_ID register
    gpio_reg |= 0x00014000;  // set the appropriate GPIOs tx
    gpio_reg |= 0x00040000;  // set the appropriate GPIOs rx
    dwt_write32bitreg(GPIO_CTRL_ID,gpio_reg); // write the modified value back
...
uint16 dwt_readtempvbat(uint8 fastSPI)
{
    ...
    if(fastSPI == 1)
    {
    	delay_ms(1); //if using PLL clocks(and fast SPI rate) then this Sleep is needed
