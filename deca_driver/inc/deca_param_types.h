...
extern agc_cfg_struct agc_config ;

//SFD threshold settings for 110k, 850k, 6.8Mb standard and non-standard
extern uint16 sftsh[NUM_BR][NUM_SFD]; 

extern uint16 dtune1[NUM_PRF];

#define XMLPARAMS_VERSION	(1.17f)

extern uint8 pll2_config[NUM_CH][5];
extern uint8 pll2calcfg;
extern uint8 rx_config[NUM_BW];
extern uint32 tx_config[NUM_CH];
extern uint8 dwnsSFDlen[NUM_BR];				//length of SFD for each of the bitrates
extern uint32 digital_bb_config[NUM_PRF][NUM_PACS];
extern uint8 chan_idx[NUM_CH_SUPPORTED];
...
extern uint16 lde_replicaCoeff[PCODES];
